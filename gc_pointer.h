...
public:
    ...
    static int refContainerSize() { return ref_container.size(); }
...
Pointer<T,size>::Pointer(T *t) {
    ...
    // if t is already in gclist (<=> ref_container), then
    // increment its reference count.
    // Otherwise, add it to the list (<=> ref_container).
    if ( p != ref_container.end() )
        p->ref_count++; // increment ref count
    else {
        // create and store this entry
        PtrDetails<T> gcObj(t, size);
        ref_container.push_front(gcObj); // insert before the head of the list
    }
...
T* Pointer<T, size>::operator=(T *t)
{
    ...
    if ( p != ref_container.end() )
        ...
        // Create and store this entry.
        PtrDetails<T> gcObj(t, size);
        ref_container.push_front(gcObj);
    }
